...
import 'package:location_menu_new/search/restaurant_search_screen.dart';
...
class _ExhibitionBottomSheetState extends State<ExhibitionBottomSheet>
  ...
  Widget child2() {
    print(events[0].assetName);
    return _buildFullItem(events[0]);
  }
  ...
  List<Widget> children() {
    ...
    for (Event event in events) {
      cc.add(_buildFullItem(event));
    }
    for (Event event in events) {
      cc.add(_buildIcon(event));
      // cc.add(SizedBox(
      //   width: 3,
      // ));
    }
    ...
    return Positioned(
      ...
      child: GestureDetector(
        onTap: () {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => RestaurantSearchScreen()),
          );
        },
        child: ClipRRect(
          ...
          child: Image.asset(
            ...
            color: Color.fromRGBO(0, 167, 212, 1),
          ),
        ),
      ),
    );
...
final List<Event> events = [
  Event('home.png', 'Go To Home ', '4.20-30'),
  Event('qr.png', 'Menu Qr Scan', '4.20-30'),
  Event('searching.png', 'Search By Resturant Name', '4.28-31'),
];
